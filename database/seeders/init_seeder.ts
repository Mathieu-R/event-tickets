...
export default class extends BaseSeeder {
	async run() {
		...
		for (const event of data) {
			const owner = await Owner.firstOrCreate({ name: event.owner.name }, { abbreviation: event.owner.abbreviation });
			...
			await Ticket.createMany(event.tickets.map((ticket) => ({ ...ticket, eventId: createdEvent.id })));
		}
	}
}
